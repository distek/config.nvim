...
function Util.OpenTerm()
	if Util.TermWin ~= 0 and vim.api.nvim_win_is_valid(Util.TermWin) then
		...
		return
	elseif
		Util.TermWin ~= 0 and not vim.api.nvim_win_is_valid(Util.TermWin)
	then
		vim.cmd("vsplit")
		Util.TermWin = vim.api.nvim_get_current_win()
...
function Util.RenameTerm()
	...
	vim.ui.input(
		{ prompt = "New Name:", default = Util.Terms[id].Name },
		function(input)
			if input == nil then
				return
			end
...
vim.api.nvim_create_autocmd({ "VimLeave" }, {
	...
	callback = function()
		...
		vim.api.nvim_exec2(
			"!rm -f " .. "/tmp/tmux-vim_" .. Util.TermSessionID,
			{}
		)
	end,
})
